    ...
    <?php
        ...
        if (sizeof ($tp) > 0 || sizeof ($tc) > 0) {
            ...
            foreach ((array) $tp as $parent_oid) {
                ...
                echo ('<li><a href="<!--root-->ticket/' . $parent->oid . '">' . htmlspecialchars ($parent->GetProp ('name')) . '</a></li>');
            }
            foreach ((array) $tc as $child_oid) {
                $child = new Ticket ($child_oid);
                echo ('<li><a href="<!--root-->ticket/' . $child->oid . '">' . htmlspecialchars ($child->GetProp ('name')) . '</a></li>');
            }
            echo ("</ul><hr />");
        }
    ?>
    <p class="col2"><b>Priority</b>: <?php echo (htmlspecialchars ($tobj->GetProp ('priority'))); ?></p>
